import { armarTributoDesdeOrigen, tasaPorDefecto, type OrigenTributo } from './tributo-desde-origen.helper';
...
describe('Percepción desde comprobantes de venta', () => {
  it('toma cliente, sucursal e importes del comprobante y aplica la tasa del régimen', () => {
    const r = armarTributoDesdeOrigen('percepcion', { regimen: '01', fechaEmision: '2026-09-17', origenes: [{ id: 10 }] }, [venta()]);
  ...
  it('suma varios comprobantes del mismo cliente y redondea a centavos', () => {
    const r = armarTributoDesdeOrigen(
      'percepcion',
      { regimen: '03', fechaEmision: '2026-09-17', origenes: [{ id: 10 }, { id: 11, fechaOperacion: '2026-09-15' }] },
  ...
  it('exige que el comprobante esté aceptado por SUNAT', () => {
    expect(() => armarTributoDesdeOrigen('percepcion', { regimen: '01', fechaEmision: '2026-09-17', origenes: [{ id: 10 }] }, [venta({ nombre_estado_sunat: 'PENDIENTE' })]))
      .toThrow('no está aceptado por SUNAT');
    expect(() => armarTributoDesdeOrigen('percepcion', { regimen: '01', fechaEmision: '2026-09-17', origenes: [{ id: 10 }] }, [venta({ nombre_estado_sunat: null })]))
      .toThrow('sin emitir');
  });
  ...
  it.each([
    ...
    expect(() => armarTributoDesdeOrigen('percepcion', { regimen: '01', fechaEmision: '2026-09-17', origenes: [{ id: 10 }] }, [venta(overrides)]))
      .toThrow(mensaje);
  });

  it('rechaza comprobantes de distintos clientes, repetidos o inexistentes', () => {
    const base = { regimen: '01', fechaEmision: '2026-09-17' };
  ...
  it('valida las fechas de cobro contra emisión del comprobante y de la percepción', () => {
    expect(() => armarTributoDesdeOrigen('percepcion', { regimen: '01', fechaEmision: '2026-09-17', origenes: [{ id: 10, fechaOperacion: '2026-09-01' }] }, [venta()]))
      .toThrow('anterior a su emisión');
    expect(() => armarTributoDesdeOrigen('percepcion', { regimen: '01', fechaEmision: '2026-09-17', origenes: [{ id: 10, fechaOperacion: '2026-09-20' }] }, [venta()]))
      .toThrow('posterior a la emisión');
  });

  it('una tasa explícita reemplaza la del régimen; sin tasa por defecto exige indicarla', () => {
    ...
    expect(() => armarTributoDesdeOrigen('percepcion', { regimen: '09', fechaEmision: '2026-09-17', origenes: [{ id: 10 }] }, [venta()])).toThrow('tasa');
    expect(tasaPorDefecto('retencion', '02')).toBe(6);
  });
});

describe('Retención desde compras', () => {
  it('descuenta la retención del importe a pagar y no exige estado SUNAT', () => {
    const r = armarTributoDesdeOrigen('retencion', { regimen: '01', fechaEmision: '2026-09-17', origenes: [{ id: 20 }] }, [compra()]);
  ...
  it('exige proveedor con RUC y factura con serie y número', () => {
    expect(() => armarTributoDesdeOrigen('retencion', { regimen: '01', fechaEmision: '2026-09-17', origenes: [{ id: 20 }] }, [compra({ documento_contraparte: '12345678' })]))
      .toThrow('proveedor con RUC');
    expect(() => armarTributoDesdeOrigen('retencion', { regimen: '01', fechaEmision: '2026-09-17', origenes: [{ id: 20 }] }, [compra({ numero: null })]))
      .toThrow('no tiene serie y número');
    expect(() => armarTributoDesdeOrigen('retencion', { regimen: '01', fechaEmision: '2026-09-17', origenes: [{ id: 20 }] }, [compra({ con_tributo: true })]))
