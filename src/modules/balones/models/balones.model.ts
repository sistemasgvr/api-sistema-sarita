...
import type { BalonEtiquetaDatos } from '../services/balon-etiqueta-pdf.generator';
import {
  CreateBalonesDto,
  AprobarBajaBalonDto,
  DarBajaBalonDto,
...
export class BalonesModel {
  ...
  obtenerPorId(id: number) {
    return this.db.callFunctionJson<AuthSingleResult>('bal_obtener_balon', [
      id,
    ]);
  }

  obtenerEtiqueta(id: number) {
    return this.db.callFunctionJson<AuthSingleResult<BalonEtiquetaDatos>>(
      'bal_obtener_etiqueta_balon',
      [id],
    );
  }
  ...
  listarSolicitudesBaja(filtros: FiltroPaginacionDto) {
    return this.db.callFunctionJson<AuthListResult>(
      'bal_listar_solicitudes_baja',
      [filtros.buscar ?? '', filtros.limite ?? 10, filtros.offset, 'PENDIENTE'],
    );
  }

  aprobarBaja(idBaja: number, dto: AprobarBajaBalonDto) {
    return this.db.callFunctionJson<AuthSingleResult>(
      'bal_aprobar_baja_balon',
      [idBaja, dto.idUsuarioAutoriza ?? null, dto.idUsuarioAuditoria ?? null],
    );
  }

  rechazarBaja(idBaja: number, dto: RechazarBajaBalonDto) {
    return this.db.callFunctionJson<AuthSingleResult>(
      'bal_rechazar_baja_balon',
      [
        idBaja,
  ...
  listarEstadoHistorial(idBalon: number, filtros: FiltroEstadoHistorialDto) {
    return this.db.callFunctionJson<AuthListResult>(
      'bal_listar_estado_historial',
      [idBalon, filtros.limite ?? 50, filtros.offset],
    );
  }
