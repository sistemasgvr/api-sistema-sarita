...
-- Function: bal_listar_alquileres_vencidos_notificar
-- Overloads: 1
-- Generated: 2026-09-02T21:31:03.562Z
DROP FUNCTION IF EXISTS bal_listar_alquileres_vencidos_notificar(p_fecha date);

CREATE OR REPLACE FUNCTION bal_listar_alquileres_vencidos_notificar(p_fecha date DEFAULT NULL::date)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
    ...
    FROM (
        SELECT json_build_object(
            ...
            'dias_vencido', (v_fecha - COALESCE(per.fecha_fin, a.fecha_fin_pactada))
        ) AS row_data,
        a.id,
        ...
        WHERE a.estado = 1
          -- Sin id_estado: se trata como vigente (datos legacy / incompletos)
          AND COALESCE(ea.nombre, 'ACTIVO') = 'ACTIVO'
          AND COALESCE(per.fecha_fin, a.fecha_fin_pactada) IS NOT NULL
          AND COALESCE(per.fecha_fin, a.fecha_fin_pactada) < v_fecha
    ) t;
