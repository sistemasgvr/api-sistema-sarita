...
BEGIN
    ...
    FROM (
        SELECT
            ...
            -- ORIGEN (directo): de dónde viene "c"
            ...
            -- DESTINO (búsqueda inversa): a qué comprobante dio lugar "c"
        ...
        LEFT JOIN ven_comprobante cd ON cd.id_comprobante_origen = c.id AND cd.estado = 1
...
select * from ven_listar_comprobantes();
/* CREATE OR REPLACE FUNCTION ven_listar_comprobantes(
    p_busqueda VARCHAR DEFAULT '',
    p_limite INTEGER DEFAULT 10,
    p_offset INTEGER DEFAULT 0,
