-- ============================================================
-- Migracion: conversion VSD -> Boleta/Factura no valida estado
--            del cilindro que ya salio con la VSD
-- Fecha: 2026-09-16
--
-- Bug: al emitir Boleta/Factura desde una venta sin documento,
-- ven_crear_comprobante validaba el id_balon del detalle como si
-- fuera una venta nueva y fallaba con "El cilindro no esta
-- disponible para venta (estado prestado cliente)". El cilindro
-- ya salio con la VSD (la conversion no mueve stock ni reserva),
-- el CPE solo lo documenta.
--
-- Fix: en conversion (v_es_conversion_vsd) se omite la validacion
-- de estado para los cilindros que ya estan en el detalle de la
-- VSD origen. Un cilindro distinto al del origen sigue validandose.
-- ============================================================
BEGIN;
