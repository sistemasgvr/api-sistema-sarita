BEGIN;
ALTER TABLE doc_salida ADD COLUMN IF NOT EXISTS fecha_emision_gre date;
-- Solo las guías que ya se enviaron conservan su fecha histórica conocida.
UPDATE doc_salida SET fecha_emision_gre = fecha::date
WHERE fecha_emision_gre IS NULL AND (ticket_sunat IS NOT NULL OR emitido_sunat);
CREATE TABLE IF NOT EXISTS doc_gre_intento (
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_doc_gre_intento_abierto ON doc_gre_intento(id_doc_salida)
WHERE estado <> 'RECHAZADO';
CREATE TABLE IF NOT EXISTS doc_gre_consulta (
 id bigserial PRIMARY KEY,
