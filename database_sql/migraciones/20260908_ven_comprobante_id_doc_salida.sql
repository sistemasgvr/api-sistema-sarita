-- ============================================================
-- Migracion: comprobante expone id_doc_salida vigente
-- Fecha: 2026-09-08
--
-- ven_obtener_comprobante y ven_listar_comprobantes incluyen la orden
-- de salida vigente de la venta para que el UI muestre «Ver orden»
-- en vez de «Generar» cuando ya existe.
--
-- Aplicar con:
--   node database_sql/scripts/apply-migration.js database_sql/migraciones/20260908_ven_comprobante_id_doc_salida.sql
-- ============================================================
...
BEGIN
    ...
    FROM (
        SELECT
            ...
            -- muestra Â«Ver ordenÂ» en vez de Â«GenerarÂ» cuando ya existe.
    ...
    -- `es_linea_garantia` marca las lÃ­neas que el POS antiguo insertaba para
    -- cobrar la garantÃ­a dentro de la venta. Desde este cambio ya no se crean,
    -- pero los comprobantes emitidos antes las conservan: la bandera deja que el
    -- detalle, el ticket y el mapper de SUNAT las traten como garantÃ­a sin que
    -- cada uno repita la comparaciÃ³n de texto por su cuenta. Es el mismo
    ...
    FROM (
        SELECT
            ...
            (COALESCE(d.descripcion, '') ~* 'garant[iÃ­]a') AS es_linea_garantia,
            d.estado,
            d.fecha_creacion,
    ...
    -- Fase 3: desglose del cobro. Solo las lÃ­neas reales de ven_comprobante_pago;
    -- si la venta no tiene ninguna, el array va vacÃ­o y el frontend cae al
    ...
    -- PrÃ©stamos de cilindro nacidos de esta venta, con TODOS sus balones: no se
    -- filtra por id_estado del detalle ni por fecha_devolucion a propÃ³sito. El
    -- comprobante documenta quÃ© cilindros salieron con esa venta; que uno ya
    -- haya vuelto no lo borra de lo que se entregÃ³ ese dÃ­a, solo cambia la
    ...
    -- GarantÃ­as ligadas a esta venta. Se llega a ellas por tres caminos porque
    -- ven_garantia no guarda id_comprobante: por el prÃ©stamo, por el alquiler o
    -- por el movimiento de cobro, que sÃ­ apunta al comprobante.
    -- `monto_cobrado_comprobante` es lo que se cobrÃ³ en ESTA venta; los demÃ¡s
    -- montos son el estado vigente de la garantÃ­a (para pantalla, no para el
    -- ticket impreso, que debe quedar como foto del dÃ­a de emisiÃ³n).
