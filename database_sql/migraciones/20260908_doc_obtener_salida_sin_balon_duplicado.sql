-- ============================================================
-- Migracion: doc_obtener_salida no duplica balon venta+prestamo
-- Fecha: 2026-09-08
--
-- Si el detalle de la venta ya trae id_balon, no se agrega de nuevo
-- desde el prestamo ENTREGADO. Evita listar el mismo cilindro dos
-- veces en la orden de salida.
--
-- Aplicar con:
--   node database_sql/scripts/apply-migration.js database_sql/migraciones/20260908_doc_obtener_salida_sin_balon_duplicado.sql
-- ============================================================
...
BEGIN
    ...
    IF v_id_venta IS NOT NULL THEN
        ...
        --              vez de mostrar quÃ© se vendiÃ³ originalmente. Cuando el
        --              Ã­tem ya trae id_balon, esa fila representa el cilindro
        --              y el gas despachado.
        --   PRESTAMO â€” cilindros entregados en prÃ©stamo por esa misma venta
        --              que NO aparezcan ya en el detalle de la venta. Si el
        --              gas se vendiÃ³ ligado al mismo cilindro, repetirlo aquÃ­
        --              duplicaba el balÃ³n en la orden de salida.
            ...
            WHERE pr.id_comprobante_venta = v_id_venta
              ...
              AND NOT EXISTS (
                  ...
                  WHERE vd_bal.id_comprobante = v_id_venta
                    ...
                    AND COALESCE(vd_bal.descripcion, '') !~* 'garant[iÃ­]a'
              )
        ) t;
    ELSE
        SELECT COALESCE(json_agg(row_to_json(t) ORDER BY t.item), '[]'::JSON) INTO v_detalle
