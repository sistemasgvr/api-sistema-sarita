-- ============================================================
-- Migracion: culminar la entrega cierra el ciclo de custodia
-- Fecha: 2026-09-09
--
-- doc_crear_desde_venta deja los cilindros de la venta en PENDIENTE_ENVIO
-- ("siguen siendo nuestros y siguen en el almacen, pero ya estan comprometidos").
-- Nada en el sistema los sacaba de ahi: 21 de 44 cilindros vivos estaban
-- atascados en ese estado, algunos ya con id_almacen NULL y cliente_ubicacion
-- puesto, o sea fisicamente entregados pero marcados como pendientes.
--
-- El reparto nacido de una venta no pasa por el movimiento de inventario de
-- doc_generar_salida (esa rama solo corre con id_venta IS NULL, porque el
-- movimiento lo creo la venta), asi que el unico punto del flujo que sabe que
-- la entrega ocurrio de verdad es culminar la entrega. Aqui se cierra.
--
-- Como en doc_crear_desde_venta, se hace con UPDATE directo y no por
-- inv_registrar_movimiento: no se mueve inventario -el stock ya salio con la
-- venta- sino que se corrige la situacion logistica del mismo cilindro.
--
-- Solo se tocan los cilindros que siguen en PENDIENTE_ENVIO. Los que ya estan
-- en otro estado se respetan, y los de baja o robados nunca "se entregan".
--
-- Aplicar con:
--   node database_sql/scripts/apply-migration.js database_sql/migraciones/20260909_age_culminar_entrega_custodia.sql
-- ============================================================
