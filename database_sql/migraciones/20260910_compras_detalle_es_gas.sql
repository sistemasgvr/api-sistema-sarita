-- ============================================================
-- Migración: com_obtener_compra expone es_gas en el detalle
-- Fecha: 2026-09-10
--
-- El editor de una compra existente valida la cantidad según la unidad de
-- medida, y las unidades tipo UNID solo admiten enteros. Los gases se venden y
-- se compran en fracciones (kg, m3) aunque la línea quede con una U.M. de las
-- estrictas, así que sin este dato el formulario rechazaba cantidades válidas
-- al crear la compra (donde sí conoce es_gas del producto) y las aceptaba al
-- editarla, o al revés. La lista de líneas nuevas ya usa es_gas; el detalle
-- guardado no lo traía.
--
-- No cambia la firma: es un campo más en el JSON del detalle.
--
-- Aplicar con:
--   node database_sql/scripts/apply-migration.js database_sql/migraciones/20260910_compras_detalle_es_gas.sql
-- ============================================================
