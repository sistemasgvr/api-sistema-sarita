-- ============================================================
-- Migración: bal_aplicar_lote_protocolo_balones engancha la ficha a la orden
-- Fecha: 2026-09-22
--
-- La base tenía la versión de 3 parámetros, pero hay dos llamadores que ya
-- pasan el cuarto:
--   · el API — lotes-protocolo.model.ts manda dto.idDocSalida, así que aplicar
--     una ficha a cilindros desde una orden fallaba con
--     "function bal_aplicar_lote_protocolo_balones(...) does not exist";
--   · bal_finalizar_recarga_planta — invoca la función con p_id_recarga_planta
--     como cuarto argumento (ese id ES una doc_salida de RECARGA_PLANTA_EXTERNA).
--
-- Qué cambia: p_id_doc_salida opcional. Si viene, escribe
-- doc_salida.id_lote_protocolo — también cuando todavía no hay cilindros que
-- aplicar — y lo devuelve en el JSON. El resto del cuerpo no se toca.
--
-- El DROP de la firma de 3 parámetros es necesario: conviviendo con la nueva
-- (que tiene DEFAULT en el cuarto) una llamada de 3 argumentos sería ambigua.
-- Los llamadores que pasan 3 — bal_crear_movimiento_recarga y
-- bal_actualizar_movimiento_recarga — resuelven contra la nueva por el DEFAULT.
-- ============================================================
